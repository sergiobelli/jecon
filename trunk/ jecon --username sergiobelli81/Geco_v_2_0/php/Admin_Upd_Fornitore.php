...
/*		
	...
	$fornitore = getRow($suppliers_table, "ID_Fornitore", $ID_Fornitore);
		
	// controllo unicita' della PI se modificata
	if ( !($fornitore['PI'] == $form['PI']) && !(empty($form['PI'])) ) // ha cambiato la PI ma non e' vuota
		{
		if (getRow ( $suppliers_table, "PI", $form["PI"])) // c'e' un altro fornitore con la stessa PI
			$message .= "Esiste gi&agrave; una registrazione con la Partita Iva che hai inserito!<br>";	
		}

	// controllo unicita' del CF se modificata	
	if ( !($fornitore['CF'] == $form['CF']) && !(empty($form['CF'])) ) // ha cambiato il cf ma non e' vuoto
		{
		if (getRow ( $suppliers_table, "CF", $form["CF"])) // c'e' un altro fornitore con la stesso CF
	...
	if ( !($fornitore['CF'] == $form['CF']) && (empty($form['CF'])) ) // ha eliminato il cf
		{

		if ( ($fornitore['PI'] == $form['PI']) && (empty($fornitore['PI'])) ) //ha tolto il cf ma la PI era vuota 
			$message .= "Se elimini il Codice Fiscale devi inserire una Partita IVA!<br>";	

		if ( !($fornitore['PI'] == $form['PI']) && !(empty($form['PI'])) ) // se ha inserito una diversa PI devo controllare se e' unica
			{
			if (getRow ( $suppliers_table, "PI", $form["PI"])) // c'e' un altro fornitore con la stessa PI
				$message .= "Esiste gi&agrave; una registrazione con la Partita Iva che hai inserito!<br>";	
			}
...
/*
	...
	if ( !($fornitore['E_mail'] == $form['E_mail']) && !(empty($form['E_mail'])) ) // ha cambiato la mail ma non e' vuota
		{
		if (getRow ( $suppliers_table, "E_mail", $form["E_mail"])) // c'e' un altro fornitore con la stessa email
...
*/
	if ( $message == "") // nessun errore	
		{
		$result = supplierUpdate($ID_Fornitore, $form["Ragione_Sociale"], $form["Cognome"], $form["Nome"], $form["PI"], $form["CF"], $form["Indirizzo"], $form["Citta"], $form["CAP"], $form["Provincia"], $form["Telefono"], $form["Telefono2"], $form["Telefono3"], $form["Fax"], $form["Cell"], $form["Password"], $form["Web"], $form["E_mail"]);
...
<input type="Hidden" name="ID_Fornitore" value="<?php print $ID_Fornitore?>">
<div align="center"><font class="FacetFormHeaderFont">Dati Fornitore</font></div>
