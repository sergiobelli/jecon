...
if (isset($actionflag) && $actionflag=="insert")
	...
	if ( empty($form["Cognome"]) || empty($form["Nome"]) /*||
		...
		// controllo se e' una persona fisica o giuridica => cod fisc o PI
/*
	if (!empty($form['Ragione_Sociale']))	// e' una persona giuridica, voglio la PI
		...
		// if (!empty($form['PI']))
			// $form['CF'] = $form['PI'];	// modifica rimandata
		}		
...
*/
	if ( $message == "") // nessun errore	
		{
		// aggiorno il db con il nuovo Fornitore
		supplierInsert ($form["Ragione_Sociale"], $form["Cognome"], $form["Nome"], $form["PI"], $form["CF"], $form["Indirizzo"], $form["Citta"], $form["CAP"], $form["Provincia"], $form["Telefono"],  $form["Telefono2"], $form["Telefono3"], $form["Fax"], $form["Cell"], $form["E_mail"], $form["Password"], $form["Web"]);
...
<div align="center"><font class="FacetFormHeaderFont">Nuovo Fornitore</div>
