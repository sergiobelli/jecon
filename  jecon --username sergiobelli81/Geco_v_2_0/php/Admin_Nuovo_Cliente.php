...
if (isset($actionflag) && $actionflag=="insert")
	...
	if (empty($form["Cognome"]) || empty($form["Nome"]) /*|| // empty($form["PI"]) || // empty($form["CF"]) || empty($form["Indirizzo"]) || empty($form["Citta"]) || empty($form["CAP"]) || empty($form["Provincia"]) || empty($form["Telefono"])*/)
        {
...
*/		
	if ( $message == "") // nessun errore	
		...
		clientInsert ($form["Ragione_Sociale"], $form["Cognome"], $form["Nome"], $form["PI"], $form["CF"], $form["Indirizzo"], $form["Citta"], $form["CAP"], $form["Provincia"], $form["Telefono"],  $form["Telefono2"], $form["Telefono3"], $form["Fax"], $form["Cell"], $form["E_mail"], $form["Password"], $form["Web"]);
...
<div align="center"><font class="FacetFormHeaderFont">Nuovo Cliente</div>
